...
try:
    import resource  # Cuma ada di Unix/Linux/macOS (termasuk Termux).
except ImportError:  # Windows tidak punya modul stdlib 'resource'.
    resource = None
...
def _pakai_ram_mb_windows() -> float:
    """Working set size proses saat ini di Windows, lewat WinAPI psapi.

    Dipakai sebagai pengganti modul `resource` yang tidak ada di Windows.
    Memakai ctypes (bawaan Python) supaya tidak perlu dependency tambahan
    seperti psutil, konsisten dengan filosofi bot ini di platform lain.
    """
    import ctypes
    ...
    penghitung = _ProcessMemoryCounters()
    penghitung.cb = ctypes.sizeof(_ProcessMemoryCounters)
    handle_proses = kernel32.GetCurrentProcess()
    if not psapi.GetProcessMemoryInfo(handle_proses, ctypes.byref(penghitung), penghitung.cb):
        raise ctypes.WinError(ctypes.get_last_error())
    return penghitung.WorkingSetSize / 1024 / 1024


def _pakai_ram_mb() -> float:
    """Memori (RSS/working set) yang dipakai proses saat ini, dalam MB.

    Sengaja memakai modul stdlib `resource` di Unix/Termux/macOS (bukan
    psutil) agar bot tetap ringan tanpa dependency tambahan yang perlu
    dikompilasi di Termux. Di Windows, `resource` tidak tersedia, jadi
    dipakai WinAPI lewat ctypes sebagai gantinya (lihat _pakai_ram_mb_windows).
    Kalau kedua cara gagal/tidak didukung, kembalikan 0.0 saja — panel info
    startup tidak boleh sampai bikin bot crash.
    """
